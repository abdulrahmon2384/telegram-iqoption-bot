...
export const LiveTradePlayCard: React.FC<LiveTradePlayCardProps> = ({
  ...
  const levelBadge = trade.managementLevel === 0 ? "Level 0 (Initial)" : `Level ${trade.managementLevel} (Martingale)`;
  ...
  return (
    <div className={`p-4 rounded-2xl border transition-all space-y-3 ${
      ...
      {/* Intermediate Checkpoint Targets (Early Exit Monitoring) */}
      {(trade.level1Time || trade.level2Time || trade.level3Time) && (
        <div className="bg-slate-900/80 border border-slate-800/80 rounded-xl p-2.5 text-xs font-mono space-y-1.5">
          <div className="flex items-center justify-between text-[11px] text-slate-400">
            <span className="font-bold text-slate-300 flex items-center gap-1">
              ...
              <span>Early Exit Checkpoints (Single Trade Management)</span>
            </span>
            {trade.earlyClosedAt && (
              <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-[10px] font-bold">
                🎯 Exited at {trade.earlyClosedAt.replace("_", " ")}
              </span>
            )}
          </div>
          <div className="grid grid-cols-3 gap-1.5 text-[11px]">
            <div className={`p-1.5 rounded-lg border text-center ${
              trade.earlyClosedAt === "LEVEL_1" 
                ? "bg-emerald-500/20 border-emerald-500/50 text-emerald-300 font-bold"
                : "bg-slate-950/60 border-slate-800 text-slate-300"
            }`}>
              <div className="text-[9px] uppercase text-slate-500">Checkpoint 1</div>
              <div>{trade.level1Time || "--"}</div>
              <div className="text-[9px] text-slate-500">{trade.earlyClosedAt === "LEVEL_1" ? "Closed in Profit" : "Take Profit Check"}</div>
            </div>

            <div className={`p-1.5 rounded-lg border text-center ${
              trade.earlyClosedAt === "LEVEL_2" 
                ? "bg-emerald-500/20 border-emerald-500/50 text-emerald-300 font-bold"
                : "bg-slate-950/60 border-slate-800 text-slate-300"
            }`}>
              <div className="text-[9px] uppercase text-slate-500">Checkpoint 2</div>
              <div>{trade.level2Time || "--"}</div>
              <div className="text-[9px] text-slate-500">{trade.earlyClosedAt === "LEVEL_2" ? "Closed in Profit" : "Take Profit Check"}</div>
            </div>
            ...
            <div className={`p-1.5 rounded-lg border text-center ${
              ...
              <div className="text-[9px] uppercase text-slate-500">Full Expiry</div>
              <div>{trade.expectedExpirationTime || trade.level3Time || "--"}</div>
              <div className="text-[9px] text-slate-500">{trade.earlyClosedAt === "EXPIRATION" ? "Settled at Expiry" : "Final Expiry Target"}</div>
            </div>
          </div>
        </div>
      )}
